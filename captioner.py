...
class CaptionerApp:
    ...
    def record_audio(self, mic_id, speaker_id):
        """Record audio and transcribe"""
        try:
            # Use device default sample rate to avoid invalid sample rate errors
            device_info = sd.query_devices(mic_id)
            target_sr = 16000
            block_duration = 2  # Process audio in ~2-second chunks

            self.root.after(0, lambda: self.status_var.set("Recording - Listening for audio..."))

            # Open stream without forcing samplerate to avoid Invalid sample rate errors
            with sd.InputStream(device=mic_id, channels=1) as stream:
                ...
                while self.is_recording:
                    try:
                        # Read audio data directly (compute frames using the stream samplerate)
                        frames = int(sample_rate * block_duration)
                        audio_data, _ = stream.read(frames)

                        if audio_data is not None and len(audio_data) > 0:
                            audio_data = audio_data.astype(np.float32).squeeze()

                            # Check if audio has content (lower threshold for better detection)
                            max_val = np.max(np.abs(audio_data))

                            if max_val > 0.001:  # Much lower threshold
                                silence_count = 0
                                audio_buffer.append(audio_data)
                                self.root.after(0, lambda: self.status_var.set("Recording - Processing audio..."))
                            else:
                                silence_count += 1
                                # Process when we accumulate audio and hit silence
                                if audio_buffer and silence_count >= 2:
                                    self.root.after(0, lambda: self.status_var.set("Recording - Transcribing..."))
                                    ...
                                    try:
                                        # Normalize audio
                                        max_audio = np.max(np.abs(full_audio))
                                        if max_audio > 0:
                                            full_audio = full_audio / max_audio * 0.9

                                        # Resample to target sampling rate in-memory if needed
                                        if sample_rate != target_sr:
                                            from scipy.signal import resample
                                            num_samples = int(len(full_audio) * target_sr / sample_rate)
                                            resampled = resample(full_audio, num_samples)
                                        else:
                                            resampled = full_audio

                                        resampled = resampled.astype(np.float32)

                                        # Pass numpy array directly to Whisper to avoid ffmpeg dependency
                                        ...
                                        if text:
                                            timestamp = datetime.now().strftime("%H:%M:%S")
                                            caption_line = f"[{timestamp}] {text}\n"

                                            self.root.after(0, self.append_caption, caption_line)
                                            self.caption_text += caption_line
                                        ...
                                        import traceback
                                        traceback.print_exc()
                                        self.root.after(0, lambda: self.status_var.set(f"Recording - Error: {str(e)[:30]}"))
